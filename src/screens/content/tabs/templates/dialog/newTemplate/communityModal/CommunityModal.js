...
const CommunityModal = props => {
  const { isOpen, onClose, values } = props;
  ...
  if (values.metaType === 'MARKETPLACE_CATEGORY') metaType = 'categoryId';

  useEffect(() => {
    buildingService.fetchBuildingBy(metaType, values.metaValue).then(response => {
      setCheckedCommunities(response.map(community => community.id));
    });
  }, []);
