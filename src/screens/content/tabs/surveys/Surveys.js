...
import classes from './Survey.style';
...
import { deleteSurvey, fetchSurveys } from '../../../../reducers/surveys/surveys.actions';
...
import NewSurveyDialog from './dialog/newSurvey/NewSurveyDialog';
...
const Surveys = props => {
  ...
  const handleFetchSurvey = useCallback(
    async page => {
      ...
      try {
        await dispatch(fetchSurveys(null, page));
      } catch (error) {
      } finally {
        setIsLoading(false);
      }
  ...
  const renderMoreOptionsMenu = () => {
    return (
      ...
      >
        ...
        <MenuItem
          onClick={
            () => handleToggleDeleteModal(deleteItemId)
          }>
          {t('Delete')}
        </MenuItem>
      </Menu>
  ...
  return (
    <>
      ...
      {isOpenDeleteModal && (
        ...
        <Box width="100%" pb={3}>
            <SurveyHeader>
              <MaterialTypography className={classes.containerText}>{t('Survey List')}</MaterialTypography>
                ...
                <Table size="small" aria-label="a dense table">
                  <TableHead>
                    <TableRow className={styles.headerRow}>
                      ...
                      <TableCell>{t('Public Since')}</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {surveys && surveys.length > 0 && surveys.map((row) => (
                      <TableRow key={row.title} className={styles.tableRow}>
                        ...
                        <TableCell>
                          {row.isPublished ? moment(row.publishedAt).format('DD/MM/YYYY') : '-'}
                        </TableCell>
...
export default withRouter(withTranslation('translations')(withStyles(classes)(Surveys)));
